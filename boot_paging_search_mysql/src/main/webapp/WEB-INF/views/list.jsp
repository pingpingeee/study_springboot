   ...
   <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
      ...
      <body>
         ...
         <form method="get" id="searchForm">
            <select name="type">
               <option value="" <c:out value="${pageMaker.criteriaDTO.type == null ? 'selected':''}"/>>전체</option>
               <option value="T" <c:out value="${pageMaker.criteriaDTO.type eq 'T' ? 'selected':''}"/>>제목</option>
               <option value="C" <c:out value="${pageMaker.criteriaDTO.type eq 'C' ? 'selected':''}"/>>내용</option>
               <option value="W" <c:out value="${pageMaker.criteriaDTO.type eq 'W' ? 'selected':''}"/>>작성자</option>
               <option value="TC" <c:out value="${pageMaker.criteriaDTO.type eq 'TC' ? 'selected':''}"/>>제목 or 내용</option>
               <option value="TW" <c:out value="${pageMaker.criteriaDTO.type eq 'TW' ? 'selected':''}"/>>제목 or 작성자</option>
               <option value="TCW" <c:out value="${pageMaker.criteriaDTO.type eq 'TCW' ? 'selected':''}"/>>제목 or 내용 or 작성자</option>
            </select>

            <!-- Criteria 를 이용해서 키워드 값을 넘김 -->
            <input type="text" name="keyword" value="${pageMaker.criteriaDTO.keyword}">
            <!-- <input type="hidden" name="pageNum" value="${pageMaker.criteriaDTO.pageNum}"> -->
			<!-- 전체 검색 중 4 페이지에서 88 키워드로 검색 시 안 나올 때 처리 -->
         ...
         <div class="div_page">
            <ul>
               ...
               <c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
                  <!-- <li>[${num}]</li> -->
                  <!-- <li ${pageMaker.criteriaDTO.pageNum == num ? "style='color: red;'":""}>[${num}]</li> -->
         ...
         <form id="actionForm" action="list" method="get">
            <input type="hidden" name="pageNum" value="${pageMaker.criteriaDTO.pageNum}">
            <input type="hidden" name="amount" value="${pageMaker.criteriaDTO.amount}">
			<!-- 페이징 검색 시 페이지 번호를 클릭 할 때 필요한 파라미터 -->
